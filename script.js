const allInputKeys = [
  ...
  { input: document.querySelector(".input9") },
  { input: document.querySelector(".input10") },
];
...
const TIME_CHALLENGE = 3; //seconds
...
function timerBar() {
  timerId = setInterval(() => {
    if (currentProgBar < 0) {
      ...
      currentStep = 1;
      step.textContent = `${currentStep}/5`;
...
function getCodeSequence() {
  return (codeSequenceArray = new Array(MAX_LETTER)
    .fill()
...
function handleEventKey(event) {
  ...
  if (
    isStart &&
    LETTERS.includes(letterInput) &&
    userInput.length < MAX_LETTER
  ) {
    ...
    if (userInput.every((e, i) => e === currentSequence[i])) {
      ...
      if (userInput.length === MAX_LETTER) {
